...
function displayQuestion() {
    ...
    let shuffledArray: number[] = [0,1,2]; 
    shuffle(shuffledArray);
    for (let i = 0; i < 3; i++) {

        (<HTMLButtonElement>document.getElementById("choice" + i)).disabled = false;
...
function checkAnswer(answer: number) {
    let category: qCategory = quiz[quiz.currentCategory];
    let question: qQuestion = category[quiz.currentQuestion];
