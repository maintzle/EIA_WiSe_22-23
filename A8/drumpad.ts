namespace Drumpad {
    ...
    let isBeatPlaying: boolean = false;
    let beat: number = 0;
    ...
    function playBeat(_event?: Event): void {
        if (!isBeatPlaying) {
            // start beat
            beat = setInterval(function () {
                for (let counter: number = 0; counter < sounds.length; counter++) {
                    playSample(counter);
                }
            }, 500);
    ...
    function changeIcon() {
        let icon: Element = document.querySelector("#playbutton");
    ...
    function hndRemix(_event: Event): void {
        if (isBeatPlaying) {
            playBeat();
        }
    ...
    function playRemix(): void {
        let rndSound: number = (Math.random() * 9);
